# Submission for labs044

This directory contains the submission for the problem **labs044**.
...
| ====== |  |
| Total Runtime | 465.1220 |
| Time to Solution | 209.9334 |
| CPU Runtime | 59535.6143 |
| GPU Runtime | 0 |
| QPU Runtime | 0 |
| Other HW Runtime | 0 |
| ====== |  |
| Remarks | LABS is unconstrained: every ±1 sequence is feasible, so # Feasible Runs = # Runs. One run = one independent dSB restart from random initial momenta; the reported value is best-of-19712, found in 1 of them. No post-processing of any kind was applied — no local search, no 1-opt or tabu polish — so this is raw dSB output. Run as a depth portfolio: the 420 s budget was split evenly across n_steps ∈ {2500, 20000, 160000}, each arm receiving 140 s. The reported value came from the n_steps = 20000 arm. Total Runtime is measured, not budgeted, and exceeds the nominal budget because an arm's deadline is tested before a batch starts rather than during it. CPU Runtime is wall time x the 128 cores the process ran on; threads and cores are 1:1 throughout, so this is measured machine time rather than an oversubscription artefact. Optimality Bound is N/A: dSB proves nothing. |
