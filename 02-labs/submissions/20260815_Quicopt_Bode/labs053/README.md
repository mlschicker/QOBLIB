...
| ====== |  |
| Total Runtime | 391.840000 |
| Time to Solution | 291.000000 |
| CPU Runtime | 15760.080000 |
| GPU Runtime | 0 |
| QPU Runtime | 0 |
| Other HW Runtime | 0 |
| ====== |  |
| Remarks | A run has two parts. Work done for that run alone takes 101 s wall and 1210 s CPU on average. A preparation step, shared by all 5 runs of this instance, takes 291 s wall and 14550 s CPU. The runtimes above charge that shared step to every run, so they say what one sequence costs from a cold start, and they do not shrink as more runs are added. All 5 runs together actually used 20600 s CPU. That is less than 5 times CPU Runtime, because the shared step ran once rather than 5 times and the runs ran side by side. Successful runs are those that reached the reported objective exactly. |
