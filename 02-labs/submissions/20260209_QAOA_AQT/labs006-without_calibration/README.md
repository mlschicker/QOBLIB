# Submission for labs006-without_calibration

This directory contains the submission for the problem **labs006-without_calibration**.

| Field | Value 1 |
| --- | --- |
| Problem | LABS (N = 6) |
| Submitter | Angelika Widl (Math.Tec), Daniel Egger (IBM), Juris Ulmanis (AQT), Christoph Regner (Math.Tec) |
| Date | Nov. 17, 2025 |
| ====== |  |
| Reference | N/A |
| Best Objective Value | 7 |
| Optimality Bound | N/A |
| ====== |  |
| Modeling Approach | PUBO |
| # Decision Variables | 6 |
| # Binary Variables | 6 |
| # Integer Variables | 0 |
| # Continuous Variables | 0 |
| # Non-Zero Coefficients | 13 |
| Coefficients Type | integer |
| Coefficients Range | {1,2} |
| ====== |  |
| Workflow | QAOA with p = 1 (200 shots per run) |
| Algorithm Type | stochastic |
| # Runs | 25 |
| # Feasible Runs | 25 |
| # Successful Runs | 25 |
| Success Threshold | 0 |
| ====== |  |
| Hardware Specifications | CPU: HPC University of Innsbruck (Leo 5), QPU: AQT IBEX |
| ====== |  |
| Total Runtime | 280.306 |
| CPU Runtime | 0.13686 |
| GPU Runtime | N/A |
| QPU Runtime | 273.605 |
| Other HW Runtime | N/A |
| ====== |  |
| Remarks | without calibration; ground state probability: 25.22% |
